...
fn process_png_with_custom_color(qrcode: &QRCode, filename: &str) {
    ...
    let resized_img =
        imageops::resize(&img, 512, 512, imageops::FilterType::Nearest);
...
fn process_resized_qrcode(qrcode: &QRCode, filename: &str) {
    let png = qrcode.to_png(512);
    let img: RgbaImage =
        ImageBuffer::from_raw(512, 512, png.into_raw()).unwrap();
    let resized_img =
        imageops::resize(&img, 256, 256, imageops::FilterType::Nearest); // Example resizing
    save_and_remove_file(resized_img.into_raw(), filename, 256, 256);
}

fn save_and_remove_file(
    data: Vec<u8>,
    filename: &str,
    width: u32,
    height: u32,
...
fn remove_file(filename: &str) {
    match fs::remove_file(filename) {
        ...
        Err(e) => {
            println!("🦀 File removal failed: ❌ {}: {}", filename, e)
        }
    }
}
